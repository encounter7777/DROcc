...
from embodiedscan.utils.typing_config import ConfigType, SampleList
...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def __init__(self,
                 ...
                 batchwise_inputs: bool = False) -> None:
        super(Det3DDataPreprocessor,
              self).__init__(mean=mean,
                             std=std,
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        if self.batchwise_inputs:
            ...
            if 'bboxes_3d' in data_samples[0].gt_instances_3d:
                assert isinstance(data_samples[0].gt_instances_3d.labels_3d,
                                  list)
            ...
            if 'eval_ann_info' in data_samples[0] and data_samples[
                    0].eval_ann_info is not None:
    ...
    def _get_pad_shape(self, data: dict) -> List[Tuple[int, int]]:
        ...
        # Process data with `default_collate`.
            ...
            pad_h = int(np.ceil(_batch_inputs.shape[1] /
                                self.pad_size_divisor)) * self.pad_size_divisor
            pad_w = int(np.ceil(_batch_inputs.shape[2] /
                                self.pad_size_divisor)) * self.pad_size_divisor
    ...
    def voxelize(self, points: List[Tensor],
        ...
        if self.voxel_type == 'hard':
            ...
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                ...
                res_voxels = res[inds]
                if self.batch_first:
                    res_voxel_coors = F.pad(res_voxel_coors, (1, 0),
                                            mode='constant',
                                            value=i)
                    data_sample.batch_idx = res_voxel_coors[:, 0]
                else:
                    res_voxel_coors = F.pad(res_voxel_coors, (0, 1),
                                            mode='constant',
                                            value=i)
    ...
    def sparse_quantize(self,
        ...
        """
        _, indices, inverse_indices = np.unique(self.ravel_hash(coords),
                                                return_index=True,
                                                return_inverse=True)
