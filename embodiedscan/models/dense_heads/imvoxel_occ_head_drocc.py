...
class ImVoxelOccHead_drocc(BaseModule):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(ImVoxelOccHead_drocc, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.pointcroatt = nn.ModuleList()
        ...
        for i in range(len(self.in_channels)):
            ...
            pointcroatt = CrossSwin3DBlock(dim=self.in_channels[i], num_heads=self.swin_num_heads[i],
                                           window_size=self.window_size[i],
                                           shift=self.shift[i])
            self.pointcroatt.append(pointcroatt)
    ...
    def forward(self, img_volumes, point_volumes, input_metas):
        ...
        for i in range(scales):
            ...
            pointout = self.pointcroatt[i](point_volumes[i], img_volumes[i])
            imgouts.append(imgout)
            pointouts.append(pointout)

        outs = []

        for i in range(scales):
            if i==0:
                out = imgouts[scales-i-1]+pointouts[scales-i-1]
            else:
                out = imgouts[scales-i-1]+pointouts[scales-i-1]+self.upconvs[i-1](outs[i-1])
            outs.append(out)

        for i in range(scales):
            occ_preds.append(self.occ[i](outs[i]))
