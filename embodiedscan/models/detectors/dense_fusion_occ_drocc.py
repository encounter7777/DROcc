...
class DenseFusionOccPredictor_drocc(BaseModel):
    ...
    def __init__(self,
        ...
        self.imgdown = nn.ModuleList()
        self.pointdown = nn.ModuleList()
        for i in range(self.scales):
            imgdown = nn.Conv3d(in_channels=256, out_channels=256, kernel_size=3, stride=2, padding=1)
            self.imgdown.append(imgdown)
            pointdown = nn.Conv3d(in_channels=256, out_channels=256, kernel_size=3, stride=2, padding=1)
            self.pointdown.append(pointdown)
    ...
    def extract_feat(self, batch_inputs_dict: dict,
        ...
        for i in range(self.scales - 1):
            img_volumes.append(self.imgdown[i](img_volumes[i]))
            point_volumes.append(self.pointdown[i](point_volumes[i]))

        return img_volumes, point_volumes, torch.stack(valid_preds).float()
